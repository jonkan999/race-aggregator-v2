...
Google Funding Choices cookie consent and AdSense Auto ads are restored for markets with `googleAdsEnabled: true` in `config/deploy-markets.json` (`fi`, `lt`, `dk`, `ee`, `de`, `pl`, and `nl`). Other markets stay ad-free.
