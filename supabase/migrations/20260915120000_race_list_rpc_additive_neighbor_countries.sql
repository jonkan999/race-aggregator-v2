-- Additive neighboring-country overlay for the main race calendar.
-- Map checkboxes pass p_neighbor_countries so domestic races stay visible while
-- selected origins are included. Exclusive /neighbors hub filters still use
-- p_origin_country / p_include_neighboring.

drop function if exists public.get_races_list_page (
  text,
  integer,
  integer,
  text,
...
create or replace function public.get_races_list_page (
  ...
  p_neighbor_countries text[] default null
) returns json
language sql
stable
security invoker
    ...
    where
      ...
      and (
        (
          ...
          and coalesce(cardinality(p_neighbor_countries), 0) = 0
        ...
        or (
          ...
          and (
            (
              coalesce(nullif(btrim(r.origin_country), ''), p_country_code) = p_country_code
              and (
                p_county is null
                or btrim(p_county) = ''
                or r.county ilike '%' || p_county || '%'
            ...
            or (
              nullif(btrim(r.origin_country), '') is not null
              and lower(btrim(r.origin_country)) in (
                select
                  lower(btrim(code))
                from
                  unnest(p_neighbor_countries) as code
                where
