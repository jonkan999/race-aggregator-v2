...
- Lithuania race-detail URLs are ASCII `/begimo_puslapiai/{domain}/`. The Unicode `/bėgimo_puslapiai/:path*` path is a one-off 308 in `config/redirects/lt-begimo-puslapiai.json`, merged into `vercel.json` at build/deploy, not generated by `generate-market-routes.mjs`.
...
- Netherlands race-detail URLs are ASCII `/looppaginas/{domain}/`. Live listing is `/hardloopkalender/`, cities `/hardloopkalender/steden/`, English `/en/race-calendar/` and `/en/race-pages/`. Fryslân county URLs stay ASCII `/hardloopkalender/fryslan`. Apex-only (`www.hardlooplijst.nl` 404s on live).
...
- The **interactive** neighbor filter on the main calendar is the map `.neighboring-countries-control` overlay: domestic races stay visible, and checking a neighbor includes that origin on both the map and the list (`p_neighbor_countries` on `get_races_list_page`). Default remains domestic-only.
- The county `<select>` neighbor optgroup **navigates** to `/neighbors/` or `/neighbors/{code}/` when the host snapshot has those foreign rows, otherwise to the sibling market's production race-list URL. It does not replace the map overlay with a foreign-only in-place filter.
- Local `/neighbors/{code}/` pages still list foreign races from the host snapshot when those rows exist. Map/list overlay options come from configured live neighbors plus any extra `origin_country` values in the snapshot, so the controls stay visible even when DK/SE snapshots are currently domestic-only.
- Neighbor race payloads belong in collector-owned `final_races_w_neighbors.json` / `_int.json` (English neighbor races within `neighbor_distance_threshold_km` of host cities, tagged with `origin_country`). Aggregator does not invent that distance filter. Empty DK/SE neighbor files need a **race-collector-v2** re-output + sync before those overlays have races to show.
